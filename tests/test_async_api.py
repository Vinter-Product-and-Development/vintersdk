import pytest
import httpx
from vintersdk import VinterAPIAsync
...
async def testget_active_data():
    ...
    with patch.object(
        ...
        result = await api.get_active_data(symbol="waves-usd-p-d")
        assert result == expected_output


@pytest.mark.asyncio
async def testget_active_data_invalid():
    api_key = "my_api_key"
    asset_type = "multi_assets"
    api = VinterAPIAsync(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.AsyncClient()
    ...
    with patch.object(
        ...
        with pytest.raises(ValueError):
            await api.get_active_data(symbol="waves-usd-p-d")


@pytest.mark.asyncio
async def test_get_multi_current_rebalance_weight():
    api_key = "my_api_key"
    asset_type = "multi_assets"
    api = VinterAPIAsync(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.AsyncClient()
    ...
    with patch.object(
        ...
        result = await api.get_multi_current_rebalance_weight(
            symbol="waves-usd-p-d"
        )
        assert result == expected_output
...
@pytest.mark.asyncio
async def test_get_multi_current_rebalance_weight_invalid():
    api_key = "my_api_key"
    asset_type = "multi_assets"
    api = VinterAPIAsync(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.AsyncClient()
    ...
    with patch.object(
        ...
        with pytest.raises(ValueError):
            await api.get_multi_current_rebalance_weight(
                symbol="waves-usd-p-d"
            )


@pytest.mark.asyncio
async def test_get_multi_current_rebalance_weight_invalid_type():
    api_key = "my_api_key"
    asset_type = "single_assets"
    api = VinterAPIAsync(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.AsyncClient()
...
@pytest.mark.asyncio
async def test_get_single_contributions():
    api_key = "my_api_key"
    asset_type = "single_assets"
    api = VinterAPIAsync(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.AsyncClient()
    ...
    with patch.object(
        ...
        result = await api.get_single_contributions(symbol="waves-usd-p-d")
        assert result == expected_output


@pytest.mark.asyncio
async def test_get_single_contributions_invalid_type():
    api_key = "my_api_key"
    asset_type = "multi_assets"
    api = VinterAPIAsync(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.AsyncClient()
    ...
    with patch.object(
        ...
        with pytest.raises(ValueError):
            await api.get_single_contributions(symbol="waves-usd-p-d")


@pytest.mark.asyncio
async def test_get_single_contributions_invalid():
    api_key = "my_api_key"
    asset_type = "single_assets"
    api = VinterAPIAsync(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.AsyncClient()
...
@pytest.mark.asyncio
async def test_get_multi_previous_rebalance_date():
    api_key = "my_api_key"
    asset_type = "multi_assets"
    api = VinterAPIAsync(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.AsyncClient()
    ...
    with patch.object(
        ...
        result = await api.get_multi_previous_rebalance_date(
            symbol="waves-usd-p-d"
        )
        assert result == expected_output
...
@pytest.mark.asyncio
async def test_get_multi_previous_rebalance_date_invalid():
    api_key = "my_api_key"
    asset_type = "single_assets"
    api = VinterAPIAsync(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.AsyncClient()
    ...
    with patch.object(
        ...
        with pytest.raises(ValueError):
            await api.get_multi_previous_rebalance_date(symbol="waves-usd-p-d")


@pytest.mark.asyncio
async def test_get_multi_previous_review_date():
    api_key = "my_api_key"
    asset_type = "multi_assets"
    api = VinterAPIAsync(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.AsyncClient()
    ...
    with patch.object(
        ...
        result = await api.get_multi_previous_review_date(
            symbol="waves-usd-p-d"
        )
        assert result == expected_output
...
@pytest.mark.asyncio
async def test_get_multi_previous_review_date_invalid():
    api_key = "my_api_key"
    asset_type = "single_assets"
    api = VinterAPIAsync(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.AsyncClient()
    ...
    with patch.object(
        ...
        with pytest.raises(ValueError):
            await api.get_multi_previous_review_date(symbol="waves-usd-p-d")


@pytest.mark.asyncio
async def test_get_multi_next_review_date():
    api_key = "my_api_key"
    asset_type = "multi_assets"
    api = VinterAPIAsync(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.AsyncClient()
    ...
    with patch.object(
        ...
        result = await api.get_multi_next_review_date(symbol="waves-usd-p-d")
        assert result == expected_output


@pytest.mark.asyncio
async def test_get_multi_next_review_date_invalid():
    api_key = "my_api_key"
    asset_type = "single_assets"
    api = VinterAPIAsync(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.AsyncClient()
    ...
    with patch.object(
        ...
        with pytest.raises(ValueError):
            await api.get_multi_next_review_date(symbol="waves-usd-p-d")


@pytest.mark.asyncio
async def test_get_multi_next_rebalance_date():
    api_key = "my_api_key"
    asset_type = "multi_assets"
    api = VinterAPIAsync(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.AsyncClient()
    ...
    with patch.object(
        ...
        result = await api.get_multi_next_rebalance_date(
            symbol="waves-usd-p-d"
        )
        assert result == expected_output
...
@pytest.mark.asyncio
async def test_get_multi_next_rebalance_date_invalid():
    api_key = "my_api_key"
    asset_type = "single_assets"
    api = VinterAPIAsync(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.AsyncClient()
    ...
    with patch.object(
        ...
        with pytest.raises(ValueError):
            await api.get_multi_next_rebalance_date(symbol="waves-usd-p-d")


@pytest.mark.asyncio
async def test_get_multi_next_rebalance_weight():
    api_key = "my_api_key"
    asset_type = "multi_assets"
    api = VinterAPIAsync(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.AsyncClient()
    ...
    with patch.object(
        ...
        result = await api.get_multi_next_rebalance_weight(
            symbol="waves-usd-p-d"
        )
        assert result == expected_output
...
@pytest.mark.asyncio
async def test_get_multi_next_rebalance_weight_invalid():
    api_key = "my_api_key"
    asset_type = "single_assets"
    api = VinterAPIAsync(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.AsyncClient()
    ...
    with patch.object(
        ...
        with pytest.raises(ValueError):
            await api.get_multi_next_rebalance_weight(symbol="waves-usd-p-d")
...
async def test_get_data_by_date():
    ...
    with patch.object(
        ...
        result = await api.get_data_by_date(
...
@pytest.mark.asyncio
async def test_get_data_by_range_invalid():
    api_key = "my_api_key"
    asset_type = "multi_assets"
    api = VinterAPIAsync(api_key=api_key, asset_type=asset_type)
    api.httpx_client = httpx.AsyncClient()
    ...
    with patch.object(
        ...
        with pytest.raises(ValueError):
            await api.get_data_by_range(
                symbol="waves-usd-p-d", start="2021-01-01"
            )
